...
const Experience: React.FC = () => {
  return (
    <section id="experience" className="py-20 md:py-24 bg-slate-900/30 scroll-mt-24">
      <div className="container mx-auto px-6">
        <div className="text-center max-w-2xl mx-auto mb-12 md:mb-16">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <span className="text-blue-400 font-bold tracking-widest uppercase text-[10px] md:text-xs mb-2 block">History</span>
            ...
            >
              ...
              <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bento-card p-6 md:p-8 group-hover:border-blue-500/30 transition-all bg-surface/50">
                <div className="flex items-center justify-between mb-2">
                  <div className="font-bold text-blue-400 text-[10px] md:text-xs uppercase tracking-widest">{exp.period}</div>
                </div>
                <div className="text-xl md:text-2xl font-bold text-white mb-1">{exp.role}</div>
                <div className="text-base md:text-lg text-slate-300 font-medium mb-4">{exp.company}</div>
                <ul className="space-y-3">
                  {exp.description.map((item, i) => (
                    <li key={i} className="text-slate-400 text-xs md:text-sm flex gap-3">
